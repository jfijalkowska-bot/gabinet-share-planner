...
serve(async (req) => {
  ...
  try {
    ...
    // Also check for one-time payments (like Wizytówka)
    ...
    logStep("Found payment intents", { count: successfulPayments.length });

    return new Response(JSON.stringify({
      subscribed: hasActiveSub,
      product_id: productId,
      subscription_end: subscriptionEnd,
      has_one_time_payment: successfulPayments.length > 0
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
